...
const ExibicaoProdutoComEfeito = () => {
  ...
  const [estaVisivel, setEstaVisivel] = useState(false); // Estado para controlar se o elemento está visível na tela
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setEstaVisivel(true);
            // Se quiser que a animação aconteça apenas uma vez, descomente abaixo:
  ...
  return (
    <div className="bg-black text-white min-h-screen flex items-center justify-center p-8 rounded-3xl overflow-hidden">
      <div className="flex flex-col items-center rounded-3xl">
        ...
        {/* Container da Bolha + Imagem */}
        <div className="relative mb-10 rounded-3xl">
          {/* Halo azul (efeito de luz) */}
          <div 
          ...
          <div className="py-4 rounded-2xl bg-gray-800/30 p-4">
            ...
            <div className="flex gap-2 justify-center">
              <div className="bg-blue-900/50 rounded-full p-3 min-w-[60px]">
                <div className="text-2xl font-mono">{tempoRestante.dias}</div>
                <div className="text-xs text-blue-200">DIAS</div>
              </div>
              <div className="bg-blue-900/50 rounded-full p-3 min-w-[60px]">
                <div className="text-2xl font-mono">{tempoRestante.horas}</div>
                <div className="text-xs text-blue-200">HORAS</div>
              </div>
              <div className="bg-blue-900/50 rounded-full p-3 min-w-[60px]">
                <div className="text-2xl font-mono">{tempoRestante.minutos}</div>
                <div className="text-xs text-blue-200">MIN</div>
              </div>
              <div className="bg-blue-900/50 rounded-full p-3 min-w-[60px]">
                <div className="text-2xl font-mono">{tempoRestante.segundos}</div>
      ...
      {/* Animação personalizada de flutuação */}
      <style>{`
        @keyframes flutuar {
